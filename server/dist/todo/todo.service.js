...
let TodoService = exports.TodoService = class TodoService {
    ...
    async getOverdueTodosByUser(user) {
        ...
        todos.forEach(async (todo) => {
            if (todo.dueDate.getTime() < today) {
                if (!emails.includes(todo.user.email)) {
                    ...
                    common_1.Logger.log("Todo overdue: " + todo.name + " " + todo.user.email);
                }
                await this.todoRepository.update({ id: todo.id }, { overDue: true });
            }
        });
        common_1.Logger.log('checking ' + emails.length);
        return emails;
    }
    async checkUserTodosOverdue(userEmail) {
        let flag = false;
    ...
    async createTodo(todoDto, user) {
        ...
        const newtodo = await this.todoRepository.create(Object.assign(Object.assign({}, todoDto), { user: user }));
